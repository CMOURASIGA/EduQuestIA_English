...
export default async function handler(req: any, res: any) {
  ...
  const { learnerId, lessonId, contentIds } = req.body || {};
  if (typeof learnerId !== "string" || typeof lessonId !== "string" || !Array.isArray(contentIds)) {
    return res.status(400).json({ error: "Dados de progresso inválidos." });
  }
  try {
    await recordCatalogProgress({ learnerId, lessonId, contentIds: contentIds.filter((id): id is string => typeof id === "string"), outcome: "completed" });

...
export default function App() {
  ...
  const [learningDataReady, setLearningDataReady] = useState(false);
  const [creatingFirstMission, setCreatingFirstMission] = useState(false);
  const [firstMissionError, setFirstMissionError] = useState<string | null>(null);
  const startedProfileId = useRef<string | null>(null);
  ...
  useEffect(() => {
    if (!profile) return;
    setLearningDataReady(false);
    setFirstMissionError(null);
    ...
    if (storedLessons) {
      ...
      if (legacyLessons) {
        try {
          ...
          setLessons([]);
          localStorage.setItem(userLessonsKey, JSON.stringify([]));
        }
      } else {
        // New profiles always start with a mission generated by the Professor EduQuest.
        setLessons([]);
        localStorage.setItem(userLessonsKey, JSON.stringify([]));
      }
    }
    ...
    if (storedVocab) {
      ...
      if (legacyVocab) {
        try {
          ...
          setVocabulary([]);
          localStorage.setItem(userVocabKey, JSON.stringify([]));
        }
      } else {
        // The Baú is earned from completed missions, not pre-filled.
        setVocabulary([]);
        localStorage.setItem(userVocabKey, JSON.stringify([]));
      }
    }
    setLearningDataReady(true);
  }, [profile?.id]);
  ...
  const handleResetLessons = () => {
    if (confirm("Quer mesmo apagar as missões salvas neste navegador? A próxima missão será criada pela IA quando você voltar à trilha.")) {
      setLessons([]);
      if (profile) {
        localStorage.setItem(`playenglish_lessons_${profile.id}`, JSON.stringify([]));
      } else {
        localStorage.setItem("playenglish_lessons", JSON.stringify([]));
      }
      startedProfileId.current = null;
    }
  };
  ...
  const createFirstMission = async () => {
    if (!profile || creatingFirstMission) return;
    setCreatingFirstMission(true);
    setFirstMissionError(null);
    try {
      const firstMission = await createNextMission({ id: "first-mission", unitId: "start", unitTitle: "Início", title: "Primeira missão", description: "", icon: "🚀", xpReward: 0, exercises: [] });
      handleUpdateLessons([firstMission]);
    } catch (error) {
      setFirstMissionError(error instanceof Error ? error.message : "Não foi possível preparar sua primeira missão.");
    } finally {
      setCreatingFirstMission(false);
    }
  };

  useEffect(() => {
    if (!profile || !learningDataReady || lessons.length || startedProfileId.current === profile.id) return;
    startedProfileId.current = profile.id;
    void createFirstMission();
  }, [profile?.id, learningDataReady, lessons.length]);
        ...
        <main className="flex-grow flex flex-col">
          ...
          {activeTab === "trilha" && (
            <div className="grid grid-cols-1 md:grid-cols-12 gap-6 items-stretch">
              ...
              <div className="md:col-span-12 mt-6">
                ...
                {creatingFirstMission && (
                  <div className="mb-6 rounded-3xl border-2 border-indigo-100 bg-white p-6 text-center shadow-sm">
                    <p className="text-2xl">🧠✨</p>
                    <h4 className="mt-2 font-black text-indigo-900">O Professor EduQuest está preparando sua primeira missão</h4>
                    <p className="mt-1 text-sm font-semibold text-slate-500">Ela será criada conforme sua idade, objetivo e nível inicial.</p>
                  </div>
                )}

                {firstMissionError && !creatingFirstMission && (
                  <div className="mb-6 rounded-3xl border-2 border-rose-100 bg-rose-50 p-6 text-center shadow-sm">
                    <h4 className="font-black text-rose-800">Não consegui preparar a primeira missão</h4>
                    <p className="mt-2 break-words text-sm font-semibold text-rose-700">{firstMissionError}</p>
                    <button type="button" onClick={() => { startedProfileId.current = null; void createFirstMission(); }} className="mt-4 rounded-xl bg-rose-600 px-4 py-2 text-xs font-black text-white">Tentar novamente</button>
                  </div>
                )}
          ...
          {activeTab === "tutor" && (
            <div className="bg-white rounded-[40px] shadow-[0_8px_30px_rgb(0,0,0,0.04)] p-8 md:p-10 border-b-8 border-blue-500 flex flex-col gap-6">
              ...
              <TutorChat profile={profile} vocabulary={vocabulary} />
            </div>
          )}
